...
| `lock.smappee_ev_YOURSERIAL_cable_lock` | Disabled by default. Enable manually in the entity settings only if your station supports cable locking (socket version), then add to your dashboard if desired. Does not work on fixed-cable models; hardware support is not detected automatically. Unavailable without a reported lock state. Existing cable lock entities are disabled once on upgrade and can then be re-enabled. |
